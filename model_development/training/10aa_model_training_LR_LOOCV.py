...
data = load_all_data(esm_model, tau_amyloid_def, WALTZ_amyloid_def).loc["TANGO_Table2"]
y = data["amyloid"]  # 'True' = forms amyloids
X = data.drop("amyloid", axis=1)

#%% Perform LOOCV with LR
...
joblib.dump({
    'model': final_model,
    'scaler': scaler
}, '../models/10aa_model_latest.joblib')

print("Final model trained and saved as '10aa_model_latest.joblib'")
