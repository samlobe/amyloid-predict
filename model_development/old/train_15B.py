...
model_save_dir = 'models_15B'  # You can change this to your preferred directory
...
#%%
# replotting for abstract
confusion_matrices = {}

# plot the ROC curve for the ensemble model on each dataset
plt.figure(figsize=(6, 4))  # Make the figure smaller for the abstract
# markers = ['o', 's', '^']  # Define markers for each dataset
linestyles = ['-', '--', ':']  # Define different line styles for clarity in grayscale
...
for i, dataset in enumerate(datasets):
    ...
    plt.plot(fpr, tpr, linestyle=linestyles[i], markevery=0.1, label=f'{dataset} dataset')
...
plt.title('ROC Curve for Ensembled Model', fontsize=14)
plt.xlim([0, 1]); plt.ylim([0, 1.05])
plt.xticks(fontsize=12); plt.yticks(fontsize=12)
plt.legend(fontsize=11.5, loc='lower right')  # Move the legend to avoid overlapping with the plot
plt.gca().set_aspect('equal', adjustable='box')

# rename the labels to "tau dataset", "WALTZ dataset", and "TANGO dataset"
plt.gca().get_legend().get_texts()[0].set_text('tau dataset')
plt.gca().get_legend().get_texts()[1].set_text('WALTZ dataset')
plt.gca().get_legend().get_texts()[2].set_text('TANGO dataset')

# Save the improved figure
plt.tight_layout()
plt.savefig('ROC_Curve_Grayscale.png', dpi=300)  # Save at higher resolution for print quality
